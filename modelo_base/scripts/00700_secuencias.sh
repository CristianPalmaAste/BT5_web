...
psql -d $DBNAME -U $USUARIO_DB < 00720_create_secuencias_1.sql > paso

grep "create sequence" paso | sed '1,$ s/^ //g' | sed '1,$ s/$/;/g' | awk '{print $0}END{printf "\n\\q\n"}' > 00730_create_secuencias_2.sql

psql -d $DBNAME -U $USUARIO_DB < 00710_drop_secuencias.sql     >  00740_create_secuencias.txt
psql -d $DBNAME -U $USUARIO_DB < 00730_create_secuencias_2.sql >> 00740_create_secuencias.txt
