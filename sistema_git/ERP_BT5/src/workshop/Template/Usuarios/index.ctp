...
<?php echo $this->BForm->formHeader( 'usuarios' ); ?>

<div class="col-md-4 text-center btn-vars__showcase">
   <div class="btn-group block">
      <a href="<?=$this->request->webroot?>Usuarios/add" title='Nuevo registro' class="btn dark"><i class="pe-7s-plus"></i>&nbsp; Nuevo</a>
   </div>
</div>
<?php echo $this->BForm->initForm( 'usuarios',array(
        $this->Paginator->sort('id', 'id'),
        $this->Paginator->sort('username', 'username'),
        $this->Paginator->sort('password', 'password'),
