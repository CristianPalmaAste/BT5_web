<?=$this->Form->create($gre, ["class" => "form-horizontal", 'url' => '/EstadosCiviles/index', 'novalidate' => true]);?>
<?php echo $this->BForm->formHeader( 'Mantención de Estados Civiles' ); ?>

<div class="col-md-4 text-center btn-vars__showcase">
   <div class="btn-group block">
      <a href="<?=$this->request->webroot?>EstadosCiviles/add" title='Nuevo registro' class="btn dark"><i class="pe-7s-plus"></i>&nbsp; Nuevo</a>
   </div>
</div>
<?php echo $this->BForm->initForm( 'Mantención de Estados Civiles',array(
        $this->Paginator->sort('alias', 'Álias'),
...
?>
<!--<tr>
<td><?=$this->Form->text("alias", ["size" => 1, "maxlength" => 1, "class" => "stacked-input upper", "id" => "alias"]);?></td>
