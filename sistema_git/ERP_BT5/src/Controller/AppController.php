...
class AppController extends Controller{
   ...
   */
   ...
   public function genMenu($idMenuActivo=null){
      ...
      $q = $per->find("all")->where(["idperf" => $idperf])->toArray();
      ...
      foreach($q as $p) {
         ...
         if ($s[0]["idprogpadre"]!=""){
            $subMenu[$s[0]["idprogpadre"]][$idprog] = ["idprog" => $idprog, "titulo" => $s[0]["titulo"], "idprogpadre" => $s[0]["idprogpadre"], 'link' => $s[0]["link"]];
            if( $this->request->url == $s[0]['link'] ){
               $idMenuActivo = $s[0]["idprogpadre"];
            }
         }else{
            $menu[$idprog] = $s[0]["titulo"];
         }
      }
