...
class ParametrosController extends AppController
{
    public function index()
    {
        ...
        $gre = $this->Parametros->newEntity();
        
        if ($this->request->is('post')) {
           $gre = $this->Parametros->patchEntity($gre, $this->request->data);
        }             
   ...
   if ($gre->nombrecorto!=null)
      $cond["nombrecorto like"] = $gre->nombrecorto."%";
   if ($gre->nombrelargo!=null)
      $cond["nombrelargo like"] = $gre->nombrelargo."%";
   if ($gre->descripcion!=null)
      $cond["descripcion like"] = $gre->descripcion."%";
   if ($gre->valor!=null)
      $cond["valor like"] = $gre->valor."%";

 
        $Parametros = $this->paginate($this->Parametros->find('all')->where($cond));

        $this->set("registros", $Parametros); 
    }
    ...
    public function add() {
	   ...
	   if ($this->request->is('post')) {
         ...
         $stmt = $conn->execute("SELECT NEXTVAL('para_seq');");
    ...
    public function edit($id=null) {
	   ...
	   if ($this->request->is(['post', 'put'])) {
	      ...
	      $this->Parametros->patchEntity($gre, $this->request->data);
    ...
    public function delete($id)
    {
       ...
       $registro = $this->Parametros->get($id);

       $registro->idusuaborraregistro = $session->read("idusua");
       $registro->fechaborraregistro  = date('Y-m-d H:i:s');	

       $this->Parametros->save($registro);
