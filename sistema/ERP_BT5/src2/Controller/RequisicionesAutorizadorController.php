...
class RequisicionesAutorizadorController extends AppController
{
	public function initialize()
    {
    
        $this->loadModel('Requisiciones');
    }
	...
	public function revisaAutorizacion() {
		...
		$n = $areq->find("all")->where(["idempr" => $idempr, "idperfautorizador" => $idperf])->count();
    ...
    public function index()
    {
	   ...
	   $arr = $areq->find("all")->where(["idempr" => $idempr, "idperfautorizador" => $idperf]);
	   ...
	   //Si no tiene perfiles subalternos, asigna el -99
	   if (count($perfiles)==0)
		  $perfiles = [-1];
	   ...
	   $arr = $usuaperf->find("all")->where(["idempr" => $idempr, "idperf IN" => $perfiles]);
    ...
    public function edit($id=null) {
	   ...
	   if ($this->request->is(['post', 'put'])) {
		  ...
		  try {
    	     ...
    	     if ($this->Requisiciones->save($gre)) {	
				...
				if ($gre->idereq==3)
				   $h->razonrechazo = $_REQUEST["razonrechazo"];
       ...
       $this->estadosRequisiciones([3, 4]); 	 

	   
       //$gre->idgrem = $session->read("idgrem");
       $gre->idempr = $session->read("idempr");

	   $gre->idereq = 4; //Autorizar
	   ...
	   for($i=0; $i < count($results); $i++) {	
		  ...
		  if ($idprod!="") {
			 ...
			 $results[$i]["desproducto"] = $prod->nombre;
    ...
    public function view($id=null) {
	   ...
	   $productos = TableRegistry::get("Productos");
	   $servicios = TableRegistry::get("Servv");
