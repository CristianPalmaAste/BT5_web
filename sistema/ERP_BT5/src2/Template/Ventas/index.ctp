...
/**
  ...
  * @var \App\Model\Entity\CotizacionesVenta[]|\Cake\Collection\CollectionInterface $cotizacionesfechaventa
  */
?>

<?php echo $this->BForm->formHeader( 'Ventas' ); ?>

<div class="col-md-4 text-center btn-vars__showcase">
<?php $nuevo = $this->request->webroot.$this->name.'/add'; ?>
   <?= $this->Form->button('<i class="pe-mi-add"></i> Nueva Venta',[
      'type'=>'button',
...
<?php echo $this->BForm->initForm( 'Ventas',array(
      $this->Paginator->sort('numero','Número'),
      $this->Paginator->sort('fechaventa','Fecha'),
...
<?php foreach ($cotizaciones as $cotizacion): ?>
   <tr>
      ...
      <td><?= $this->Time->format($cotizacion->fechaventa,'d/MM/Y') ?></td>
      ...
      <td>
         ...
         <?php if ($cotizacion->idesve==1) echo $this->Html->link('<i class="pe-7s-note"></i>', ['action' => 'edit', $cotizacion->id],array('escape' => false,'data-toggle'=>"tooltip", 'title'=>"Editar")); ?>
         <?php if ($cotizacion->idesve==1) echo $this->Html->link('<i class="pe-7f-trash"></i>', ['action' => 'delete', $cotizacion->id], array('confirm' => __('¿Está seguro que desea eliminar el documento ID {0}?', $cotizacion->id), 'escape' => false, 'title' => 'Eliminar')); ?>
      </td>
   </tr>
   <tr class='spacer'></tr>
<?php endforeach; ?>
