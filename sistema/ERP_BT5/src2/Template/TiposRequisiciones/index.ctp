<?=$this->Form->create($gre, ["class" => "form-horizontal", 'url' => '/TiposRequisiciones/index', 'novalidate' => true]);?>
<?php echo $this->BForm->formHeader( 'Tipos de Requisiciones' ); ?>

<div class="col-md-4 text-center btn-vars__showcase">
   <div class="btn-group block">
      <a href="<?=$this->request->webroot?>TiposRequisiciones/add" title='Nuevo registro' class="btn dark"><i class="pe-7s-plus"></i>&nbsp; Nuevo</a>
   </div>
</div>
<?php echo $this->BForm->initForm( 'Tipos de Requisiciones',array(
        $this->Paginator->sort('id', 'Id'),
        $this->Paginator->sort('descripcion', 'Descripción'),
        $this->Paginator->sort('nrocotizacsminimo', 'Nro.Míni.de Cotizs.'),
            'Acciones'
            )
            );
?>
...
<td><?=$this->Form->text("nrocotizacsminimo", ["size" => 20, "maxlength" => 20, "class" => "stacked-input upper", "id" => "nrocotizacsminimo"]);?></td>
