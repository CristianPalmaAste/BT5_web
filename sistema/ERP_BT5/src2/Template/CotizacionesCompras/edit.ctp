...
?>
	  ...
	  <table style="border:1px solid white;" border="2" cellspacing="1" cellpadding="1">
		...
		<td><?=$this->Form->select("idprov", $proveedores, [ "id" => "idprov", "style" => "color:black;", "required" => true]);?>
		</td>
		</tr>
		
		<tr><td><label for='idprov' >Observaciones</label></td>
		<td><?=$this->Form->text("observaciones", ["size" => 100, "maxlength" => 1000, "class" => "stacked-input upper", "id" => "observaciones", 'style' => 'color:black;']);?></td></tr>
...
<td align='right'><input type="text" id="preciounitario<?=$i?>" name="preciounitario<?=$i?>" value="<?=$r["preciounitario"]?>" size="11" maxlength="11" style="color:black; text-align:right" onblur="chkNo(this);"/></td>
...
<tr><td colspan='9' align='right'><button type='submit' class='ninguna' style="color:black">Grabar</button></td></tr>
</table>
