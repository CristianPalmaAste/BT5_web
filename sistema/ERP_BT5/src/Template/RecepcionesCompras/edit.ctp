...
<?php if ($pendiente!=0):?>
<td align='right'><input type="text" id="cantidad<?=$i?>" name="cantidad<?=$i?>" value="<?=$r["cantidad"]?>" size="11" maxlength="11" style="color:black; text-align:right" onblur="chkNo(this,<?=$pendiente?>);"/></td>
<?php else: ?>
<td align='right'><input type="hidden" id="cantidad<?=$i?>" name="cantidad<?=$i?>" value="<?=$r["cantidad"]==""?0:$r["cantidad"]?>"/><?=$r["cantidad"]==""?0:$r["cantidad"]?></td>
<?php endif; ?>
...
<script language="javascript">
  function chkNo(n, pendiente) {
	  if (!isNumber(n.value)) {
		 n.value="";
		 return;
	  }
	  
	  if (n.value*1.0==0) return;
	 
	  if (n.value*1.0 > pendiente*1.0) {
		 alert("¡Cantidad recepcionada no puede superar a cantidad pendiente!");
		 n.value="";
		 n.focus();
	  }
   }
   ...
   document.getElementById("frm").onsubmit=function() {	  
	  ...
	  for (i=0; i < nRows; i++) {
		 ...
		 if (cantidad.value*1.0==0) nCeros++;
	  }
	  
	  if (nCeros==nRows) {
		 alert("¡Al menos debe ingresar una cantidad no cero!");
