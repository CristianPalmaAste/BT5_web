<?=$this->Form->create($gre, ["class" => "form-horizontal", 'url' => '/Programas/index', 'novalidate' => true]);?>
<?php echo $this->BForm->formHeader( 'Programas' ); ?>

<div class="col-md-4 text-center btn-vars__showcase">
   <div class="btn-group block">
      <a href="<?=$this->request->webroot?>Programas/add" title='Nuevo registro' class="btn dark"><i class="pe-7s-plus"></i>&nbsp; Nuevo</a>
   </div>
</div>
<?php echo $this->BForm->initForm( 'Programas',array(
        $this->Paginator->sort('id'      , 'Id'),
        $this->Paginator->sort('titulo'  , 'Título'),
        $this->Paginator->sort('link'    , 'Link'),
        $this->Paginator->sort('etiqueta', 'Etiqueta'),
        $this->Paginator->sort('peso'    , 'Peso'),
            'Acciones'
            )
            );
?>
