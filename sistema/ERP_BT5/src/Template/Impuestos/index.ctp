<?=$this->Form->create($gre, ["class" => "form-horizontal", 'url' => '/Impuestos/index', 'novalidate' => true]);?>
<?php echo $this->BForm->formHeader( 'Impuestos' ); ?>

<div class="col-md-4 text-center btn-vars__showcase">
   <div class="btn-group block">
      <a href="<?=$this->request->webroot?>Impuestos/add" title='Nuevo registro' class="btn dark"><i class="pe-7s-plus"></i>&nbsp; Nuevo</a>
   </div>
</div>
<?php echo $this->BForm->initForm( 'Impuestos',array(
        $this->Paginator->sort('descripcion', 'Descripción'),
        $this->Paginator->sort('descripcioncorta', 'Descripcion corta'),
        $this->Paginator->sort('obligatorio', 'Obligatorio'),
...
<td><?=$this->Form->text("obligatorio", ["size" => 1, "maxlength" => 1, "class" => "stacked-input upper", "id" => "obligatorio"]);?></td>
