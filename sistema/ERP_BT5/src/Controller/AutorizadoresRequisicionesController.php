...
class AutorizadoresRequisicionesController extends AppController
{
    public function index()
    {
        ...
        $gre = $this->AutorizadoresRequisiciones->newEntity();
        
        if ($this->request->is('post')) {
           $gre = $this->AutorizadoresRequisiciones->patchEntity($gre, $this->request->data);
        }             
        ...
        $AutorizadoresRequisiciones = $this->paginate($this->AutorizadoresRequisiciones->find('all')
		                            ->where($cond));

		//debug($AutorizadoresRequisiciones);
		
        $this->set("registros", $AutorizadoresRequisiciones); 
    ...
    public function add() {
	   ...
	   if ($this->request->is('post')) {
		 ...
		 $ar = TableRegistry::get('AutorizadoresRequisiciones');
...
//	        if ($this->AutorizadoresRequisiciones->save($gre)) {
...
//	        $this->Flash->error(__('No se puede crear registro.')); 
    ...
    public function edit($id=null) {
       ...
       $gre = $this->AutorizadoresRequisiciones->get($id);
	   ...
	   if ($this->request->is(['post', 'put'])) {
         ...
         $sql = "delete from autorizadores_requisiciones where idempr=$idempr and idperfautorizador=".$gre->idperfautorizador;
		 ...
		 $ar = TableRegistry::get('AutorizadoresRequisiciones');
		 foreach($e as $idperfautorizado) {		 
			$gre2 = $this->AutorizadoresRequisiciones->newEntity();
...
//	        $this->Flash->error(__('No se puede crear registro.')); 
    ...
    public function delete($id)
    {
       ...
       $registro = $this->AutorizadoresRequisiciones->get($id);

       //$registro->idusuaborraregistro = $session->read("idusua");
       //$registro->fechaborraregistro  = date('Y-m-d H:i:s');	

       $this->AutorizadoresRequisiciones->save($registro);
	...
	public function llena_perfiles_libres($idempr, $idperfautorizador=null) {
	   ...
	   if ($idperfautorizador==null)
          ...
          $sql = "select * from perfiles where id not in (select idperfautorizado from autorizadores_requisiciones where idempr=$idempr and idperfautorizador=$idperfautorizador) order by descripcion";
	...
	public function llena_perfiles_sel($idempr, $idperfautorizador) {
       ...
       $sql = "select * from perfiles where id in (select idperfautorizado from autorizadores_requisiciones where idempr=$idempr and idperfautorizador=$idperfautorizador) order by descripcion";
	...
	public function llena_perfiles_nuevos($idempr) {
       ...
       $sql = "select * from perfiles where id not in (select idperfautorizador from autorizadores_requisiciones where idempr=$idempr) order by descripcion";
    ...
    public function excel() {
       ...
       header('Content-Disposition: attachment; filename=autorizadores_requisiciones.xlsx');
...
;
       ...
       $rows = $this->AutorizadoresRequisiciones->find('all')->where(['idempr' => $session->read('idempr')]); 
